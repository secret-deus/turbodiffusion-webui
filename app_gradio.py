...
def generate_video(
    ...
    if not MANAGER.is_loaded() or MANAGER.cfg.name != preset_name:
        MANAGER.load(PRESETS[preset_name])
    ...
    status = f"✅ Done in **{sec}s** | seed={seed} | saved: `{out_path}`"
    log_text = "\n".join(logs[-200:])

    return str(out_path), status, log_text

with gr.Blocks(title="TurboDiffusion WebUI (Wan2.1 T2V)") as demo:
    gr.Markdown("# TurboDiffusion WebUI (Engine Mode)\n"
                "Model warm-load + multi-infer reuse")
        ...
        with gr.Tab("Generate"):
            with gr.Row():
                with gr.Column(scale=4):
                    ...
                    attn_choices = ["sla", "original"]
                    if has_spargeattn():
                        attn_choices.insert(1, "sagesla")
                    attention_type = gr.Dropdown(attn_choices, value="sla", label="Attention Type")
            ...
            run_btn.click(
                fn=generate_video,
                inputs=[preset, prompt, num_steps, num_frames, num_samples,
                        seed_mode, seed, attention_type, sla_topk, sigma_max,
                        fps, default_norm],
                outputs=[out_video, status_md, log_box],
                concurrency_id="gpu",
                concurrency_limit=1,
            )
        ...
        with gr.Tab("Models"):
            with gr.Row():
                with gr.Column(scale=3):
                    ...
                    validate_out = gr.Textbox(label="Checkpoint Status", lines=6)
